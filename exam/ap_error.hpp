 ...
 *
 * If you want to throw your own exception type you can use the macro
 ...
 *
 * struct Stack_pool_exception: public std::runtime_error{
 *   using std::runtime_error::runtime_error; // using the same constructors
 *                                            // of the parent
 * };
 *
 * AP_ASSERT(1>2,Stack_pool_exception); // it will throw Stack_pool_exception
 *
 ...
 *   << "all " << "what do you want to write to debug " << a << " " << b << " " \
 *   << c << " and " << d << std::endl;
 *
...
#define AP_ERROR_SOFT(...)                                                          \
  SELECT_MACRO(__VA_ARGS__, _AP_ERROR2_SOFT, _AP_ERROR1_SOFT, dummy)(__VA_ARGS__)
...
#define _AP_ERROR2_SOFT(cond, exception_type)                                  \
  if (!(cond))                                                                 \
  ::internal::AssertHelper<exception_type>{} =                                 \
      internal::MessageHandler{}

#define _AP_ERROR1_SOFT(cond) _AP_ERROR2_SOFT(cond, std::runtime_error)

#define AP_ASSERT(...)                                                         \
  SELECT_MACRO(__VA_ARGS__, _AP_ASSERT2, _AP_ASSERT1, dummy)(__VA_ARGS__)

#define AP_ASSERT_SOFT(...)                                                    \
  SELECT_MACRO(__VA_ARGS__, _AP_ASSERT_SOFT_, _AP_ASSERT1_SOFT, dummy)(__VA_ARGS__)

#ifndef NDEBUG
#  define _AP_ASSERT_(cond, exception_type) AP_ERROR(cond, exception_type)
#  define _AP_ASSERT_SOFT_(cond, exception_type) AP_ERROR_SOFT(cond, exception_type)
#else
#  define _AP_ASSERT_(cond, exception_type)                                    \
    internal::NullStream {}
#  define _AP_ASSERT_SOFT_(cond, exception_type)                               \
    internal::NullStream {}
#endif
...
#define _AP_ASSERT1_SOFT(cond) _AP_ASSERT2_SOFT(cond, std::runtime_error)
